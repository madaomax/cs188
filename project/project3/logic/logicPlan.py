...
def sentence3() -> Expr:
    ...
    return conjoin(PacmanAlive_1 % disjoin(PacmanAlive_0 & ~PacmanKilled_0, ~PacmanAlive_0 & PacmanBorn_0), ~(PacmanAlive_0 & PacmanBorn_0), PacmanBorn_0)
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    if pl_true(inverse_statement, model=assignments):
        return False
    else:
        return True
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model1 = findModel(PropSymbolExpr(pacman_str, x1, y1, time=1) & conjoin(KB))
    model2 = findModel(~PropSymbolExpr(pacman_str, x1, y1, time=1) & conjoin(KB))
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t)):
                ...
                # cannot be proven, ambiguous situation
                possible_locations.append((x, y))
...
def mapping(problem, agent) -> Generator:
    ...
    # Add initial position of Pacman
    KB.append(PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0))
    # There is no wall at Pacman's start position
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            # print("CHECK", entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)), entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)))
            if known_map[x][y] == 1 or entails(conjoin(KB), PropSymbolExpr(wall_str, x, y)):
                # guaranteed to have a wall at (x, y)
                KB.append(PropSymbolExpr(wall_str, x, y))
                known_map[x][y] = 1
            elif known_map[x][y] == 0 or entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y)):
                # guaranteed to not have a wall at (x, y)
                KB.append(~PropSymbolExpr(wall_str, x, y))
                known_map[x][y] = 0
            else:
                # cannot be proven, ambiguous situation
        ...
        # take action and move to the next step
        agent.moveToNextState(agent.actions[t])
