...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variables = [GHOST0, GHOST1, PAC, OBS0, OBS1]

    # Constructing Bayes' nets, edge list: (x, y) means edge from x to y
    edges = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]

    # Construct the domain for each variable
    variableDomainsDict[PAC]  = []
    variableDomainsDict[GHOST0]  = []
    variableDomainsDict[GHOST1]  = []
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            variableDomainsDict[PAC] = variableDomainsDict[PAC] + [(x, y)]
            variableDomainsDict[GHOST0] = variableDomainsDict[GHOST0] + [(x, y)]
            variableDomainsDict[GHOST1] = variableDomainsDict[GHOST1] + [(x, y)]
            
    # each observed distance is a noisy Manhattan distance:
    variableDomainsDict[OBS0] = [x for x in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)]
    variableDomainsDict[OBS1] = [x for x in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)]
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim_var in eliminationOrder:
            currentFactorsList, joined_factor = joinFactorsByVariable(currentFactorsList, elim_var)
            if len(joined_factor.unconditionedVariables()) != 1:
                eliminated_factor = eliminate(joined_factor, elim_var)
                currentFactorsList.append(eliminated_factor)
        
        fullJoint = joinFactors(currentFactorsList)
        result = normalize(fullJoint)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            obsProb = self.getObservationProb(observation, gameState.getPacmanPosition(), ghostPosition, self.getJailPosition())
            self.beliefs[ghostPosition] = obsProb * self.beliefs[ghostPosition]
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        old_belief = self.beliefs.copy()
        self.beliefs.clear()
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos in self.allPositions:
                self.beliefs[newPos] += newPosDist[newPos] * old_belief[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % numPos])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Calculate the weights of all particles and total weight for each state.
        state_weights = DiscreteDistribution()
        for particle in self.particles:
            prob = self.getObservationProb(observation, pacPos, particle, jailPos)
            state_weights[particle] = state_weights.get(particle, 0) + prob
            
        if state_weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            # Resample the particles from the observed distribution. 
            self.particles = []
            numPos = len(self.legalPositions)
            for i in range(self.numParticles):
                self.particles.append(state_weights.sample())
